...
| `lint:md` | remark 检查 Markdown | 否 | 否 | 否 |
...
文档目录已从 Prettier 忽略，格式以 remark 为准。
...
本次已做：检查与 `--fix` 分离；`prettier` 去掉互相冲突的 `-c --write`；`lint` / `lint:circular` 并行。
...
| knip / remark 进 CI 或 `pnpm lint` | 噪音与文档格式存量未知 |
