...
describe('SearXNGClient', () => {
  ...
  it('should return empty response when body contains "Empty Results" (case insensitive)', async () => {
    mockFetch.mockResolvedValue({
      ...
      status: 500,
      statusText: 'Internal Server Error',
      text: () => Promise.resolve('{"error":"Search failed","message":"SearxNG returned Empty Results"}'),
    })
  ...
  it('should include response body in error message', async () => {
    ...
    await expect(client.search('test')).rejects.toThrow('Failed to search: 502 Bad Gateway - upstream timeout')
  })
  ...
  it('should handle text() failure gracefully', async () => {
    ...
    await expect(client.search('test')).rejects.toThrow('Failed to search: 500 Internal Server Error')
  })
})
