...
describe('ssrfSafeFetch (@pure/ssrf-safe-fetch)', () => {
  ...
  describe('SSRF protection for malicious URLs', () => {
    ...
    privateHttpUrls.forEach((url) => {
      it(`should SSRF-block private HTTP URL: ${url}`, async () => {
        mockFetch.mockImplementation(() => {
          throw new Error('DNS lookup 169.254.169.254 is not allowed. Because, It is private IP address.')
        })
  ...
  describe('error handling', () => {
    it('should throw error with descriptive message when fetch fails', async () => {
      ...
      await expect(ssrfSafeFetch('https://example.com')).rejects.toThrow('Fetch failed: Network error')
    ...
    it('should handle non-Error thrown values', async () => {
      ...
      await expect(ssrfSafeFetch('https://example.com')).rejects.toThrow('Fetch failed: String error')
    })
  ...
  describe('maxContentLength (response body cap)', () => {
    ...
    it('truncates the body once the cap is reached and stops reading further chunks', async () => {
      const chunks = [Buffer.from('a'.repeat(60)), Buffer.from('b'.repeat(60)), Buffer.from('c'.repeat(60))]
  ...
  describe('OOM regression: bounded memory under oversized response bodies', () => {
     ...
     */
    const buildHugeBodyResponse = (counter: { bytesYielded: number }, totalAvailableBytes: number) => {
    ...
    it('stops pulling chunks from the source once the cap is reached', async () => {
      ...
      const response = await ssrfSafeFetch('https://huge.example.com', {}, { maxContentLength: ONE_MB })
    ...
    itIfGc('heap delta stays bounded when a 50 MB body is fetched with a 1 MB cap', async () => {
      ...
      const response = await ssrfSafeFetch('https://heavy.example.com', {}, { maxContentLength: ONE_MB })
