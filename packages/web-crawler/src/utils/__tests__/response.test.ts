...
const createMockResponse = (body: string, options: { ok?: boolean; status?: number; statusText?: string } = {}) => {
...
describe('parseJSONResponse', () => {
  ...
  it('should throw ResponseBodyParseError for non-JSON response', async () => {
    ...
    await expect(parseJSONResponse(createMockResponse('<html><body>Error</body></html>'), 'Jina')).rejects.toThrow(
  ...
  it('should handle empty response body', async () => {
    ...
    await expect(parseJSONResponse(response, 'TestProvider')).rejects.toThrow('TestProvider returned non-JSON response')
  })
})
