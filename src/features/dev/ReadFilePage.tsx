      ...
      <div className='mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8'>
        <header className='flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end'>
          <div>
            ...
            <h1 className='text-3xl font-semibold tracking-normal text-slate-950 sm:text-4xl'>文件解析功能测试台</h1>
              ...
              <div className='grid grid-cols-2 gap-2 rounded-lg bg-slate-100 p-1'>
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition ${
                    mode === 'file' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
                >
                ...
                <button
                  ...
                  onClick={() => setMode('url')}
                  className={`inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition ${
                    mode === 'url' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
                >
              ...
              {mode === 'file' ? (
                <div className='mt-4'>
                  ...
                  >
                    ...
                    <div className='mt-4 text-sm font-medium text-slate-900'>拖拽文件到这里，或点击选择</div>
              ...
              {error ? (
                ...
                <div className='flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-slate-600'>
                  ...
                  <div>
                    ...
                    <div className='mt-1 text-sm leading-5'>选择文件或输入 URL 后开始解析，响应会显示在这里。</div>
                  </div>
                </div>
              )}
              ...
              <div className='mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4'>
                {[
                  ...
                  <div key={label} className='rounded-lg border border-slate-200 bg-slate-50 p-3'>
                    ...
                    <div className='mt-1 truncate text-sm font-semibold text-slate-950'>{summarizeValue(value)}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className='grid max-h-[58vh] gap-3 overflow-auto pr-1'>
                  {pages.length > 0 ? (
                    pages.map((page, index) => (
                      <article
                        key={`${pageTitle(page, index)}-${index}`}
