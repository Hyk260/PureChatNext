...
import { sniffBinaryBuffer, sniffBinaryFile } from '../src/utils/isBinaryContent'

describe('sniffBinaryBuffer (@pure/file-loaders)', () => {
  it('empty buffer is text', () => {
    expect(sniffBinaryBuffer(Buffer.alloc(0))).toEqual({ isBinary: false })
  })

  it('plain ASCII is text', () => {
    const buf = Buffer.from('hello world\nthis is text\n', 'utf8')
    expect(sniffBinaryBuffer(buf).isBinary).toBe(false)
  })

  it('UTF-8 CJK is text', () => {
    const buf = Buffer.from('你好,世界。这是中文。\n这也是中文。', 'utf8')
    expect(sniffBinaryBuffer(buf).isBinary).toBe(false)
  })

  it('printable base64 stays text', () => {
  ...
  it('UTF-16LE BOM → text despite zero bytes', () => {
  ...
  it('BOM-less UTF-16LE (Windows export style) → text', () => {
    const buf = Buffer.from('hello world\nthis is a longer ASCII sample\n', 'utf16le')
    expect(sniffBinaryBuffer(buf).isBinary).toBe(false)
  })

  it('BOM-less UTF-16BE → text', () => {
  ...
  it('high non-printable ratio → binary', () => {
    const bytes: number[] = []
...
describe('sniffBinaryFile (@pure/file-loaders)', () => {
  const tmpDir = path.join(os.tmpdir(), 'pure-sniff-binary-' + process.pid)
