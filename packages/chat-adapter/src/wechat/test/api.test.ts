...
describe('WechatApiClient', () => {
  ...
  describe('constructor', () => {
    ...
    it('should strip trailing slashes from base URL', async () => {
      ...
      await c.getUpdates()
      expect(mockFetch).toHaveBeenCalledWith('https://example.com/ilink/bot/getupdates', expect.anything())
    })
  })
  ...
  describe('getUpdates', () => {
    ...
    it('should send cursor in request body', async () => {
      mockFetch.mockResolvedValueOnce(jsonResponse({ ret: 0, msgs: [], get_updates_buf: 'cursor_2' }))
  ...
  describe('downloadCdnMedia', () => {
    ...
    it('should throw on CDN HTTP error', async () => {
      ...
      await expect(client.downloadCdnMedia({ aes_key: 'x', encrypt_query_param: 'AAFFtest' })).rejects.toThrow(
        'CDN download failed: 404'
      )
    })

    it('should throw when encrypt_query_param is missing', async () => {
      await expect(client.downloadCdnMedia({ aes_key: 'x', encrypt_query_param: '' })).rejects.toThrow(
        'Missing encrypt_query_param'
      )
    })
    ...
    it('should throw when AES key is valid but ciphertext is corrupt', async () => {
      ...
      mockFetch.mockResolvedValueOnce(new Response(new Uint8Array(corruptCiphertext), { status: 200 }))

      await expect(
        client.downloadCdnMedia({ aes_key: 'ABEiM0RVZneImaq7zN3u/w==', encrypt_query_param: 'AAFFtest' }, aesKeyHex)
      ).rejects.toThrow()
    })
  })
  ...
  describe('uploadCdnMedia', () => {
    it('should follow the 3-step getuploadurl → encrypt → POST CDN flow', async () => {
      ...
      const result = await client.uploadCdnMedia('user_1@im.wechat', WechatUploadMediaType.IMAGE, plaintext)
    ...
    it('should throw if getuploadurl returns no upload_param', async () => {
      ...
      await expect(client.uploadCdnMedia('u', WechatUploadMediaType.FILE, Buffer.from('x'))).rejects.toThrow(
        'empty upload_param'
      )
    })
