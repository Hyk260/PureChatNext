...
export default function DeleteUserDevPage() {
  ...
  const canDelete = preview?.found === true && confirmEmail.trim() === email.trim() && confirmEmail.trim().includes('@')
  ...
  const deleteSummary = lastResult?.action === 'delete' && lastResult.result.found ? lastResult.result.deleted : null
        ...
        <header className='flex flex-col gap-2'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-sm text-slate-600'>按邮箱查询用户并删除关联数据（仅开发环境，操作不可恢复）</p>
            </div>
          </div>
        </header>
