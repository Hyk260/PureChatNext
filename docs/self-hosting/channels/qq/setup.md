...
`CHANNEL_GATEWAY_INTERNAL_URL` 可覆盖内部回调地址。兼容周期内仍读取 `QQ_WEBHOOK_SECRET`，统一密钥优先。Webhook 模式无需启用 Gateway；保存绑定后把设置页显示的回调地址配置到 QQ 开放平台。QQ 回复使用账号在设置页保存的 OpenAI / DeepSeek 密钥，不读取服务端 `OPENAI_API_KEY` / `DEEPSEEK_API_KEY`。
