...
describe('QQAdapter', () => {
  ...
  describe('fetchAttachmentData', () => {
    it('should fetch attachment data via fetchData callback', async () => {
      ...
      vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(new Response(imageBytes, { status: 200 })))
  ...
  // ---------- passive reply msg_id ----------

  describe('postMessage passive reply', () => {
    it('should pass inbound msg_id / msg_seq to sendGroupMessage', async () => {
      const sendSpy = vi
      ...
      const threadId = 'qq:group:group_abc'
      await adapter.postMessage(threadId, { markdown: 'reply' } as any)

      expect(sendSpy).toHaveBeenCalledWith(
        'group_abc',
        expect.any(String),
        expect.objectContaining({ msgId: 'inbound_msg_99', msgSeq: 1 })
      )

      await adapter.postMessage(threadId, { markdown: 'reply2' } as any)
