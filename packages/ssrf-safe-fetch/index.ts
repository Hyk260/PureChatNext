 ...
 */
const readBodyWithCap = async (body: NodeJS.ReadableStream | null, cap: number): Promise<Uint8Array> => {
...
export const ssrfSafeFetch = async (
  ...
  try {
    ...
    const agentOptions: RequestFilteringAgentOptions = {
      allowIPAddressList:
        ssrfOptions?.allowIPAddressList ?? process.env.SSRF_ALLOW_IP_ADDRESS_LIST?.split(',').filter(Boolean) ?? [],
