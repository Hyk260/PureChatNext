...
async function getLastDbMigration(
  connection: postgres.Sql,
): Promise<DbMigrationRecord | null> {
...
export async function getMigrationSummary(
  ...
  const lastAppliedWhen = lastDbMigration
  ...
  const pendingTags = journalMigrations
    .filter(
      (migration) =>
        lastAppliedWhen === null || lastAppliedWhen < migration.when,
    )
...
export async function assertMigrationStateHealthy(
  connection: postgres.Sql,
  migrationsFolder: string,
): Promise<void> {
  const journalMigrations = readJournalMigrations(migrationsFolder);
  ...
  if (lastDbMigration && lastDbMigration.hash.length !== SHA256_HEX_LENGTH) {
    throw new Error(
      [
        ...
        '请参考 docs/DRIZZLE_SETUP.md#迁移失败 修复迁移记录后再执行。',
      ].join('\n'),
    );
  }
  ...
  const willRerunFromStart =
    lastAppliedWhen === null ||
    lastAppliedWhen === 0 ||
    lastAppliedWhen < firstJournalWhen;
  ...
  if (
    lastAppliedWhen !== null &&
    journalMigrations.length >= 2 &&
    usersExists
  ) {
    const secondLastWhen =
      journalMigrations[journalMigrations.length - 2]!.when;
