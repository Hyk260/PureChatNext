...
describe('browserless', () => {
  ...
  it('should throw NetworkConnectionError on fetch failed', async () => {
    ...
    await expect(browserless('https://example.com', { filterOptions: {} })).rejects.toThrow(NetworkConnectionError)
  })
  ...
  it('should return crawl result on successful fetch', async () => {
    process.env.BROWSERLESS_TOKEN = 'test-token'
    const longContent = 'This is a test paragraph with enough content to pass the length check. '.repeat(3)
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      statusText: 'OK',
