...
const parseScopes = (scope: string | undefined) => (scope ? scope.split(/[\s,]+/).filter(Boolean) : [])
    ...
    return {
      ...
      getUserInfo: async (tokens) => {
        ...
        return {
          ...
          emailVerified: false,
          id: unionId,
          image: profile.avatar_url ?? profile.avatar_thumb ?? profile.avatar_middle ?? profile.avatar_big,
