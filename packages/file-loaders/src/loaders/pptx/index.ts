...
export class PptxLoader implements FileLoaderInterface {
  ...
  async loadPages(filePath: string): Promise<DocumentPage[]> {
    ...
    try {
      ...
      const slideFiles: ExtractedFile[] = await extractFiles(filePath, (fileName) => slidesRegex.test(fileName))
      ...
      const pages: DocumentPage[] = slideFiles
        .map((slideFile, index) => {
          try {
            ...
            log(`Slide ${index + 1} text extracted, lines: ${lines.length}, characters: ${slideText.length}`)
...
${page.pageContent}
   ...
   */
  private createErrorPage(errorInfo: string, sourceFileName: string, sourceFilePath?: string): DocumentPage {
