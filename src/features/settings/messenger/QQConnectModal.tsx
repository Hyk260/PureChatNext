...
import { Button as AntButton, QRCode, Radio, Select, Spin } from 'antd'
import { Alert, Button, Flexbox, Input, Text } from '@pure/ui'
...
const QQConnectContent = memo<QQConnectContentProps>(({ agentId, close, gatewaySupported, model, onConnected, provider }) => {
  ...
  return (
    <Flexbox gap={18} style={{ paddingBlock: 4 }}>
      ...
      {!gatewaySupported && (
        <Alert showIcon type='info' title='当前部署仅支持 URL 回调' description='扫码和 WebSocket 需要开启内置 Channel Gateway 的本地或 Docker 单实例环境。' />
      )}
      ...
      {mode === 'qr' ? (
          ...
          {qrStatus?.status === 'waiting' && (
            <>
              <div className={styles.qrWrap}>
                ...
                <div className={styles.qrIcon}><PlatformAvatar platform='qq' size={42} /></div>
              </div>
              <Text type='secondary'>请使用手机 QQ 扫描二维码并确认连接</Text>
            </>
          )}
          {qrStatus?.status === 'binding' && <><Spin size='large' /><Text type='secondary'>授权成功，正在连接 QQ…</Text></>}
          {qrStatus?.status === 'selecting' && (
            <Flexbox gap={12} style={{ width: '100%' }}>
              <Alert showIcon type='info' title='请选择要连接的 QQ 机器人' />
              <Select options={qrStatus.appIds.map((value) => ({ label: `QQ Bot ${value}`, value }))} value={selectedAppId} onChange={setSelectedAppId} />
              <AntButton block loading={loading} type='primary' onClick={() => void handleSelection()}>连接所选机器人</AntButton>
            </Flexbox>
          )}
          {error && (
            <Flexbox
        ...
        <Flexbox gap={12}>
          ...
          <Input.Password placeholder='QQ 机器人 App Secret' value={appSecret} onChange={(event) => setAppSecret(event.target.value)} />
          <Text type='secondary' style={{ fontSize: 13 }}>
            {mode === 'websocket' ? '由内置 Gateway 维护长连接，无需公网回调地址。' : '保存后将在设置页显示需要配置到 QQ 开放平台的回调地址。'}
          </Text>
          ...
          <Flexbox horizontal justify='flex-end' gap={8}>
            ...
            <AntButton loading={loading} type='primary' onClick={() => void handleManualBind()}>注册</AntButton>
          </Flexbox>
        </Flexbox>
      )}
    </Flexbox>
...
export const QQConnectButton = memo<Omit<QQConnectContentProps, 'close'>>((props) => (
  <Button icon={<LinkIcon size={16} />} type='primary' onClick={() => openQQConnectModal(props)}>连接</Button>
))
