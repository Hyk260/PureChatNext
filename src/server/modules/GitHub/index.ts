...
export class GitHub {
  ...
  generateIdentifier(info: GitHubRepoInfo): string {
    const parts = [this.normalizeIdentifierPart(info.owner), this.normalizeIdentifierPart(info.repo)]
  ...
  async downloadRepoZip(info: GitHubRepoInfo): Promise<Buffer> {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        throw new GitHubNotFoundError(`Repository not found: ${info.owner}/${info.repo}@${info.branch}`)
      }
      log('downloadRepoZip: download failed with status=%d', response.status)
      throw new GitHubDownloadError(`Failed to download repository: ${response.status} ${response.statusText}`)
    }
  ...
  async downloadRawFile(info: GitHubRawFileInfo): Promise<string> {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        throw new GitHubNotFoundError(`File not found: ${info.owner}/${info.repo}@${info.branch}/${info.filePath}`)
      }
      throw new GitHubDownloadError(`Failed to download file: ${response.status} ${response.statusText}`)
    }
