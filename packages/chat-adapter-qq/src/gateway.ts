...
export interface QQGatewayOptions {
  ...
  shard?: [number, number]
  /** Extra headers for webhook forwarding (e.g. Authorization Bearer) */
...
export class QQGatewayConnection {
  ...
  private handlePayload(payload: QQGatewayPayload, isResume: boolean, onReady: (err?: Error) => void): void {
  ...
  private sendIdentify(): void {
    this.api
      ...
      .then((token) => {
        const identifyPayload: QQGatewayPayload = {
          d: {
            ...
            properties: {
              $browser: 'purechat-gateway',
              $device: 'purechat-gateway',
              $os: 'linux',
            },
            shard: this.shard,
            token: `QQBot ${token}`,
      ...
      .catch((err) => {
        ...
        this.openConnectionError = err instanceof Error ? err : new Error('Failed to get access token for identify')
  ...
  private sendResume(): void {
    this.api
      ...
      .catch((err) => {
        ...
        this.openConnectionError = err instanceof Error ? err : new Error('Failed to get access token for resume')
  ...
  private attemptReconnect(): void {
    ...
    this.log('Reconnecting in %dms (attempt %d/%d)', delay, this.reconnectAttempts, MAX_RECONNECT_ATTEMPTS)
