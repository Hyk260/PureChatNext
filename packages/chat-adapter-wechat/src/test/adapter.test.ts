...
import { MessageItemType, MessageState, MessageType, type WechatRawMessage } from '../types'
...
describe('WechatAdapter', () => {
  ...
  describe('parseMessage', () => {
    ...
    it('should produce metadata-only video attachment with size', async () => {
      ...
      expect(downloadSpy).not.toHaveBeenCalled()
      expect(message?.attachments).toEqual([{ mimeType: 'video/mp4', size: 12_345, type: 'video', url: '' }])
    })
  ...
  describe('postMessage', () => {
    ...
    beforeEach(() => {
      ...
      sendMessageSpy = vi.spyOn((adapter as any).api, 'sendMessage').mockResolvedValue({ ret: 0 }) as MockInstance<
        WechatApiClient['sendMessage']
      >
      sendItemSpy = vi.spyOn((adapter as any).api, 'sendItem').mockResolvedValue({ ret: 0 }) as MockInstance<
        WechatApiClient['sendItem']
      >
    ...
    it('uploads and sends an image attachment as a separate IMAGE item', async () => {
      ...
      await adapter.postMessage(threadId, {
        attachments: [{ data: bytes, mimeType: 'image/png', name: 'pic.png', type: 'image', url: '' }],
        markdown: 'check this out',
      })

      // Text should be sent first via sendMessage, image as a separate sendItem.
      expect(sendMessageSpy).toHaveBeenCalledTimes(1)
      expect(uploadSpy).toHaveBeenCalledWith('user_x@im.wechat', WechatUploadMediaType.IMAGE, bytes)
    ...
    it('routes a file attachment to a FILE item carrying file_name + len', async () => {
      ...
      await adapter.postMessage(threadId, {
        attachments: [{ data: bytes, mimeType: 'application/pdf', name: 'report.pdf', type: 'file', url: '' }],
        raw: '',
      })
    ...
    it('falls back to attachment.url when data is absent', async () => {
      ...
      vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(new Response(new Uint8Array(remoteBytes), { status: 200 })))
...
describe('downloadMediaFromRawMessage', () => {
  ...
  beforeEach(() => {
    ...
    downloadSpy = vi.spyOn(api, 'downloadCdnMedia') as MockInstance<WechatApiClient['downloadCdnMedia']>
  })
  ...
  it('falls back to CDN thumbnail when main media download fails', async () => {
    ...
    downloadSpy.mockRejectedValueOnce(new Error('main CDN failed')).mockResolvedValueOnce(thumbBytes)
  ...
  it('downloads a video', async () => {
    ...
    expect(result).toEqual([{ buffer: videoBytes, mimeType: 'video/mp4', size: 24, type: 'video', url: '' }])
  })
  ...
  it('downloads voice as audio/silk', async () => {
    ...
    expect(result).toEqual([{ buffer: voiceBytes, mimeType: 'audio/silk', type: 'audio', url: '' }])
  })
