...
describe('exa crawler', () => {
  ...
  it('should successfully crawl content with API key', async () => {
    ...
    expect(result).toEqual({
      ...
      contentType: 'text',
      length: 'This is a test article with enough content to pass the length check. '.repeat(3).length,
  ...
  it('should return undefined when no results are returned', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Exa API returned no results for URL:', 'https://example.com')
  ...
  it('should throw PageNotFoundError for 404 status', async () => {
    ...
    await expect(exa('https://example.com', { filterOptions: {} })).rejects.toThrow(PageNotFoundError)
  })
  ...
  it('should throw NetworkConnectionError for fetch failures', async () => {
    ...
    await expect(exa('https://example.com', { filterOptions: {} })).rejects.toThrow(NetworkConnectionError)
  })
  ...
  it('should rethrow unknown errors', async () => {
    ...
    await expect(exa('https://example.com', { filterOptions: {} })).rejects.toThrow('Unknown error')
  })
